...
is.throws(function () {
	r = cf.parse('<cfinvokeargument >');
}, Error, 'Missing required attributes.');

is.throws(function () {
	r = cf.parse('<cfinvokeargument name="cfinvarg">');
}, Error, 'Missing required value attributes.');

is.throws(function () {
	r = cf.parse('<cfinvokeargument value="cfinvarg">');
}, Error, 'Missing required name attributes.');

r = cf.parse('<cfinvokeargument name="cfinvokeargument_test2" value="cfinvarg2" />');
...
r = cf.parse('<cfinvokeargument name="cfinvokeargument_test3" value="cfinvarg3" omit="yes" />');
is.equal(r instanceof Object, true);
is.equal(r.tag, 'invokeargument');
...
r = cf.parse('<CFINVOKEARGUMENT OMIT="yes" NAME="cfinvokeargument_test4" VALUE="cfinvarg4" />');
is.equal(r instanceof Object, true);
is.equal(r.tag, 'invokeargument');
