...
is.throws(function () {
	r = cf.parse('<cffeed properties="#meta#" output_file="/path/to/output" xml_var="feed_xml_var">');
}, Error, 'Missing required query attribute.');

is.throws(function () {
	r = cf.parse('<cffeed action="create" query="#get_data#" output_file="/path/to/output" xml_var="feed_xml_var">');
}, Error, 'Missing required properties attribute.');

is.throws(function () {
	r = cf.parse('<cffeed action="create" query="#get_data#" properties="#meta#" >');
}, Error, 'Missing required output_file or xml_var attribute.');

r = cf.parse('<cffeed query="#query1#" properties="#meta1#" output_file="/path/to/output1">');
...
r = cf.parse('<cffeed query="#query2#" properties="#meta2#" xml_var="var1">');
is.equal(r instanceof Object, true);
is.equal(r.tag, 'feed');
is.equal(r.attributes.action, 'create');
...
r = cf.parse('<cffeed query="#query3#" properties="#meta3#" output_file="/path/to/output2" xml_var="var2">');
is.equal(r instanceof Object, true);
is.equal(r.tag, 'feed');
is.equal(r.attributes.action, 'create');
...
r = cf.parse('<cffeed action="create" query="#query4#" properties="#meta4#" column_map="#map1#" output_file="/path/to/output3" overwrite="true">');
is.equal(r instanceof Object, true);
is.equal(r.tag, 'feed');
is.equal(r.attributes.action, 'create');
