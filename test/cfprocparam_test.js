...
var r;
is.throws(function () {
	r = test.cfparser.parse('<cfprocparam value="1" />');
}, Error, "Missing required attribute");

is.throws(function () {
	r = test.cfparser.parse('<cfprocparam CFSQLtype="CF_SQL_BIT" type="in">');
}, Error, "Missing required value attribute.");

is.throws(function () {
	r = test.cfparser.parse('<cfprocparam CFSQLtype="CF_SQL_BIT" type="out">');
}, Error, "Missing required variable attribute.");

is.throws(function () {
	r = test.cfparser.parse('<cfprocparam CFSQLtype="CF_SQL_BIT" type="inout">');
}, Error, "Missing required variable attribute.");
