...
is.throws(function () {
	r = test.cfparser.parse('<cfexecute >');
}, Error, "Missing required name attribute and closing tag.");

is.throws(function () {
	r = test.cfparser.parse('<cfexecute arguments="-v"></cfexecute>');
}, Error, "Missing required name attribute.");

is.throws(function () {
	r = test.cfparser.parse('<cfexecute name="" >');
}, Error, "Empty name attribute value and closing tag.");

is.throws(function () {
	r = test.cfparser.parse('<cfexecute name="" ></cfexecute>');
