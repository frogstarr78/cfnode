...
is.throws(function () {
	r = cf.parse('<cffile fileField="sfile" destination="/tmp/dfile" />');
}, Error, 'Missing required action attribute.');

is.throws(function () {
	r = cf.parse('<cffile fileField="sfile" action="upload" >');
}, Error, 'Missing required destination attribute.');

is.throws(function () {
	r = cf.parse('<cffile action="upload" destination="/tmp/dfile" >');
}, Error, 'Missing required fileField attribute.');

r = cf.parse('<cffile action="upload" destination="/tmp/dfile" fileField="cffile_upload_test">');
