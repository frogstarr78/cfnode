...
is.throws(function () {
	r = cf.parse('<cfmailparam>');
}, Error, "Missing required attributes.");

is.throws(function () {
	r = cf.parse('<cfmailparam name="cfmail_name" file="/path/to/file" />');
}, Error, "Unexpectedly defined name and file attributes.");

r = cf.parse('<cfmailparam name="cfmailparam_test">');
...
r = cf.parse('<cfmailparam file="/path/to/file">');
is.equal(r instanceof Object, true);
is.equal(r.tag, 'mailparam');
...
r = cf.parse('<cfmailparam name="cfmailparam_test2" disposition="inline" remove="yes" content="hello world" content_id="123abc" type="plain" value="val" />');
is.equal(r instanceof Object, true);
is.equal(r.tag, 'mailparam');
...
r = cf.parse('<cfmailparam file="/tmp/path" disposition="inline" remove="yes" content="hello world" content_id="123abc" type="text" value="val" />');
is.equal(r instanceof Object, true);
is.equal(r.tag, 'mailparam');
is.equal(r.attributes.content_id, "123abc");
is.equal(r.attributes.disposition, "inline");
...
r = cf.parse('<CFMAILPARAM FILE="/tmp/path" DISPOSITION="inline" REMOVE="yes" CONTENT="hello world" CONTENT_ID="123abc" TYPE="html" VALUE="val" />');
is.equal(r instanceof Object, true);
is.equal(r.tag, 'mailparam');
is.equal(r.attributes.content_id, "123abc");
is.equal(r.attributes.disposition, "inline");
