...
is.throws(function () {
	r = test.cfparser.parse('<cfzip action="list" file="/tmp/file.zip" entrypath="/tmp/files" >');
}, Error, 'Missing required variable attribute.');

is.throws(function () {
	r = test.cfparser.parse('<cfzip action="list" variable="something" entrypath="/tmp/files" />');
}, Error, 'Missing required file attribute.');

is.throws(function () {
	r = test.cfparser.parse('<cfzip file="/tmp/file.zip" variable="something" action="list" />');
}, Error, 'Missing required entry_path attribute.');

r = test.cfparser.parse('<cfzip action="list" variable="cfzip_list_test" file="/tmp/file.zip" entrypath="/tmp/files" >');
