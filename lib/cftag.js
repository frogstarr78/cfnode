	...
	switch( me.tag ) {
      ...
      break; // property
		...
		case 'zip':
			...
			if ( ['delete', 'list', 'read', 'read_binary', 'unzip', 'zip'].indexOf(me.attributes.action) > -1 && plib.is_empty(me.attributes.file) ) {
				throw new Error("Missing required file attribute.");
			} else if ( ['read', 'read_binary'].indexOf( me.attributes.action ) > -1 && plib.is_empty(me.attributes.variable) ) {
				throw new Error("Missing required variable attribute.");
			} else if ( me.attributes.action === 'read' && plib.is_empty(me.attributes.entry_path) ) {
