	...
	switch( me.tag ) {
		...
		case 'feed':
			if ( me.attributes.action === 'create' ) {
				if ( typeof me.attributes.output_file === 'undefined' || me.attributes.output_file === '' ) {
					throw new Error("Missing required output_file attribute.");
				}
				if ( typeof me.attributes.xml_var === 'undefined' || me.attributes.xml_var === '' ) {
					throw new Error("Missing required xml_var attribute.");
				}

				if ( ( typeof me.attributes.name === 'undefined' || me.attributes.name === '' ) && 
					 (
					  ( typeof me.attributes.properties === 'undefined' || me.attributes.properties === '' ) ||
					  ( typeof me.attributes.query === 'undefined' || me.attributes.query === '' ) 
					 )
				   ) {
						throw new Error("Missing required name, or properties and query attribute.");
				}
			} else if ( me.attributes.action === 'read' ) {
			}
			...
			if ( typeof me.attributes.user_agent === 'undefined' || me.attributes.user_agent === '' ) {
				me.attributes.user_agent = 'cfNode ColdFusion';
			}
			break; // feed
		case 'file':
			switch( me.attributes.action ) {
