...
class MissingRequiredAttributeError extends Error {
  constructor(attribute, ...params) {
    ...
    if (Error.captureStackTrace) {
      Error.captureStackTrace(this, MissingRequiredAttributeError);
    }
            ...
            if ( me.attributes.action == 'query' ) {
                ...
                for (var i=0; i < require_attributes.length; i++) {
                    ...
                    if ( plib.is_empty(me.attributes[attr]) ) {
                        throw new Error("Missing required " + attr + " attribute.");
                    }
                }
            }
...
//			if ( [''].includes(me.attributes.action) && plib.is_empty(me.attributes.attributes) ) {
//				throw new Error("Missing required attributes attribute.");
//			}
